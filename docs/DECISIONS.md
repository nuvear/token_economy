...
### D18 · Delivery is full source under a time-bound license; the customer may integrate the engine *(refines D17)*
The customer receives the **entire codebase** (source), self-hosts it, and may **integrate the pricing/governance engine with their existing systems** (CPQ, CRM, ERP). Use is governed by a **time-bound commercial license**. **Why:** customers want to own and integrate the software, not consume a black box. **Consequence — enforcement reality (be honest about this):** because source is delivered, technical license checks are **good-faith enforcement + deterrence + tamper-evidence, not DRM** — a customer with source can bypass a check, so the **legal license agreement is the primary control**. The technical layer's job is: (a) a signed, offline-verifiable license a legitimate customer honors (they cannot forge the vendor signature to self-extend the term); (b) a graceful, data-preserving expiry ladder; (c) tamper-evidence via audit logging and optional license telemetry so overrun is detectable. **Architecture consequences:** the engine must be a **clean, documented, versioned, embeddable module** with a stable public interface (it is already pure — `src/engine/`), the **license layer must be separable** from the engine so embedding is friction-free, and delivery is a **branded source distribution** (not just a container). Full activity list: `docs/PRODUCTION-READINESS.md` groups 10–12.

### D19 · Sustained revenue via a vendor-signed activation key + subscriptions *(extends D18)*
Revenue is recurring, not a one-time sale. The app ships **locked**; it unlocks only with a **vendor-signed, time-bound activation key** that the vendor mints upon payment — renewal requires a fresh vendor-signed key, so **continued use requires the vendor's ongoing consent**. **Why:** the user wants sustained revenue with an unlock gated on the vendor. **Mechanism:** the vendor holds the private signing key; the delivered source contains only the public key. The customer cannot forge a valid key, so a legitimate deployment must obtain a renewed key each term (offline token drop by default; optional online activation/heartbeat for continuous consent and remote revocation). **Honest constraint (from D18 — source is delivered):** the technical unlock is **good-faith enforcement + the renewal touchpoint, not DRM** — a customer could edit out the check, which the **legal license prohibits**. Therefore durable recurring revenue is anchored in **value only the vendor can supply**, not the bypassable lock: (1) term license renewal (the key), (2) maintenance & support — updates, security patches, new LLM adapters, and the framework's **quarterly model rate-card refresh**, (3) the **curated benchmark / knowledge-base subscription** (D7/MCP) — cross-engagement intelligence the customer cannot self-generate, the strongest lock-in, (4) metered/entitlement add-ons (insights via a vendor gateway, seats, optional modules). **Consequence:** the license token carries subscription entitlements + expiry; the packaging/renewal tooling (groups 10–11) is the revenue operations backbone; the knowledge-base (D7) is repositioned as a *recurring product*, not just an end-state. Full model: `docs/MONETIZATION.md`.
