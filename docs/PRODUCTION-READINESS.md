...
4. **Phase-2 evidence loop** — unit capture, calibration, sold-vs-actual (makes the platform whole).
5. **Accessibility, cross-device QA, expanded tests, pen test** — then tag **`v1.0.0`**.

A realistic GA is the P0 + P1 set across groups 1–6 and 8, with group 7 (Phase 2) and the P2 items as the immediate fast-follow if schedule forces a cut — but a `1.0.0` without the evidence loop should be an explicit, documented decision, since "Track → Learn" is core to the product's thesis.
